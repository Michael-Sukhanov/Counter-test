...
public:
    ...
    IPbusTarget() {
        ...
        connect(updateTimer, &QTimer::timeout, this, &IPbusTarget::sync);
        ...
        qsocket->bind(QHostAddress::AnyIPv4, 50444);
    }
...
//            updateTimer->start(1000);
    ...
    void sync() { //update all parameters
        addTransaction(read, 0x0, buffer, 16);
        if (transceive()) emit statusDataReady();
    }

    bool readRegister(quint32* data, quint32 address){
        addTransaction(read, address, data, 1);
        if (transceive()) return 1;
        else return 0;
    }

    bool writeRegister(quint32 data, quint32 address) {
        addTransaction(write, address, &data, 1);
        if (transceive()) return 1;
        else return 0;
    }

    bool Non_incrementing_readRegister(quint32* data, quint32 address, quint8 nWords){
        addTransaction(nonIncrementingRead, address, data, nWords);
        return transceive();
    }
...
//    bool setPhase(quint16 phaseShift){
//        quint32 data[5] = { quint32(0x03000000+round(phaseShift/9.765625))
//                           ,0x00000000
//                           ,0x00000000
//                           ,0x00000000
//                           ,0x00002320
//        };
//        addTransaction(write,0x00002000,data,5);
//        if (transceive()) return 1;
//        else return 0;
//    }
